document.addEventListener("DOMContentLoaded", () => {
  ...
  let issuesData = []; // Source of truth for all data

  // --- 2. LOADING SPINNER CHALLENGE ---
  const showLoader = () => {
    container.innerHTML = `
  ...
  searchInput.addEventListener("input", (e) => {
    ...
    // Update the main number to reflect search results
  ...
  // --- 6. RENDER FUNCTION (With Icons & Top Borders) ---
  function displayIssues(issues) {
    ...
    issues.forEach((issue) => {
      const statusIcon = issue.status === "closed" ? "./assets/closed_status.png" : "./assets/open_status.png";
      
      // Challenge: Top border color based on status
      const borderColor = issue.status === "open" ? "border-t-green-500" : "border-t-violet-600";

      // Map labels back to your original Font Awesome style
  ...
  // --- 7. HELPER: UPDATE STATS (Numbers Only) ---
  function updateStats(filteredData, type) {
    ...
    // Show ONLY the number in the main heading
    if (totalEl) totalEl.innerText = filteredData.length;

    // Logic to update the counts next to the green/purple dots
    if (type === "open" && openCountEl) {
        ...
        if (openCountEl) openCountEl.innerText = issuesData.filter(i => i.status === "open").length;
        if (closedCountEl) closedCountEl.innerText = issuesData.filter(i => i.status === "closed").length;
    }
  }

  // --- 8. HELPER: ACTIVE BUTTON STYLING (Blue for selected, White for others) ---
  function setActiveButton(activeBtn) {
    const buttons = [btnAll, btnOpen, btnClosed];

    buttons.forEach((btn) => {
      // Set others to White/Inactive
      btn.classList.remove("bg-violet-600", "text-white", "btn-primary");
      btn.classList.add("bg-white", "text-gray-700", "border-gray-200");
    });

    // Set clicked button to Blue/Active
  ...
  function openModal(issue) {
    ...
    if(footer) footer.innerText = `Reported by ${issue.author} on ${issue.createdAt.slice(0, 10)}`;
